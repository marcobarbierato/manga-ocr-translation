...
class MangaTranslator():
    ...
    def __init__(self, master, do_ocr=True, do_translate=True):
        ...
        if self.do_translate:
            self.translator = deepl.Translator("") ### deepl API KEY goes there, obviously not to be hardcoded
    ...
    def startup_variables(self):
        self.page_counter = -1
        # Dictionary for OCR text: key is page number (starting from 0) and value is list of strings.
        self.jap_text_dict = {0:[]}
        self.eng_text_dict = {0:[]}
        # dictionary for rectangle coords. key is page number, value is list of duples with left up corner of rectangle.
        self.coord_dict = {0:[]}
        #See if there is an old file!
    ...
    def load_images(self, images_list, width=900, height=1400): #normal is w 900
    ...
    def change_page(self, increment):
        ...
        # Updates Showing Image

        # self.label.destroy()
        # self.label = Label(self.pictureframe, image=self.current_display_img,cursor='cross')
        # self.label.bind("<ButtonPress-1>", self.get_rect_coords)
        # self.label.bind("<ButtonRelease-1>", self.get_rect_coords)
        # self.label.bind("<KeyPress>", self.key)
        # self.label.bind("<MouseWheel>", lambda event: self.change_page(1) if event.delta<0 else self.change_page(-1))
        ...
        #Clears and restores boxes
        self.textbox.clear()
        self.translatebox.clear()
        try: # if the page has not been created, skips restoring
        ...
        try:
            for rect in self.coord_dict[self.page_counter]:
                ratio_w_h = [
                self.current_img.width/self.current_display_img.width(), self.current_img.height/self.current_display_img.height()]
                rect = [
                rect[0]/ratio_w_h[0],
                rect[1]/ratio_w_h[1],
                rect[2]/ratio_w_h[0],
    ...
    def key(self, event):
        ...
        if event.keysym == 'Right':
            ...
            self.eng_text_dict[self.page_counter] = [] # clears
    ...
    def get_rect_coords(self, event):
        if event.type == EventType.Button:
            ...
            # Checks if rect is too small (accidental click)
    ...
    def ocr_img(self, rect):
        ...
        # tl = ratioed_rect[0:2]
        # try:
        #     self.coord_dict[self.page_counter].append(tl)
        # except KeyError:
        #     self.coord_dict[self.page_counter] = []
        #     self.coord_dict[self.page_counter].append(tl)\
        try:
    ...
    def append_text(self, text, text_dict):
        # Appends line of text to text_dict
        try:
    ...
    def write_to_file(self):
        def dict_to_text(text_dict):
            print(text_dict)
            full_text_list = []
            for page, text_list in text_dict.items():
                full_text_list.append(f'Page {int(page)+1}\n')
        ...
        # jap_full_text = dict_to_text(self.jap_text_dict)
        # eng_full_text =  dict_to_text(self.eng_text_dict)
        self.coord_dict = order_dict(self.coord_dict)
        json_dict = {
            **{"JapPages":self.jap_text_dict},
            **{"EngPages":self.eng_text_dict}, 
            # **{'JapFullText': jap_full_text}, 
            # **{'EngFullText': eng_full_text},
            **{'RectCoords':self.coord_dict}
            }
        with open(f'{self.folder_name}.json', 'w+', encoding='utf-8') as f:
            json.dump(json_dict, f, ensure_ascii=False, indent=4)
            self.is_saved=True
        # with open(f'{self.folder_name}.txt', 'w+', encoding='utf-8') as f:
        #     f.write(jap_full_text)
...
#Maybe not really necessary. Adds some method to our textboxes
#Actually seems to be useful!
class TextBox(Text):
    def append(self, text):
        self.configure(state='normal')
        self.insert("end +1 lines", text)
        self.see('end') # scrolls to the end
...
if __name__ == '__main__':
    os.chdir(r'') # folder where images are

do_ocr = True
do_translate = True
root = Tk()
app = MangaTranslator(root, do_ocr, do_translate)
root.mainloop()
# root.destroy()